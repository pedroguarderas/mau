...
assumed, if $c_i$ is $0$ then the utility is linear otherwise is an exponential function.
...
utilities is built and saved in the local working directory, after that with `Eval.Utilities` every 
function is evaluated over the columns of the index table, the names for utilities were previously
standardized with `Stand.String`. With another file `tree.csv` the decision tree associated to the
