...
\end{eqnarray*}
where $I(x) := \int\limits_D k(x,y) \, d\mu(y)$ and $\alpha := \frac{1}{\int\limits_D\int\limits_D
k(x,y) \,d\mu(x)d\mu(y)}$, now
\begin{eqnarray*}
...
\mathbf{F}}
\end{equation*}
where $I$ is the identity matrix and $\Gamma_i$ is the matrix
\begin{equation}
